...
class IPOPTLIB_EXPORT TNLP : public ReferencedObject {
 public:
    ...
    *                     (TNLP::C_STYLE: 0-based, TNLP::FORTRAN_STYLE: 1-based; see also \ref
    * TRIPLET)
    */
   virtual bool get_nlp_info(
         Index& n, Index& m, Index& nnz_jac_g, Index& nnz_h_lag, IndexStyleEnum& index_style) = 0;
   ...
   virtual bool get_var_con_metadata(Index n, StringMetaDataMapType& var_string_md,
         IntegerMetaDataMapType& var_integer_md, NumericMetaDataMapType& var_numeric_md, Index m,
         StringMetaDataMapType& con_string_md, IntegerMetaDataMapType& con_integer_md,
   ...
   virtual bool get_bounds_info(
         Index n, Number* x_l, Number* x_u, Index m, Number* g_l, Number* g_u) = 0;
    ...
    */
   virtual bool get_scaling_parameters(Number& obj_scaling, bool& use_x_scaling, Index n,
         Number* x_scaling, bool& use_g_scaling, Index m, Number* g_scaling) {
      (void)obj_scaling;
      (void)use_x_scaling;
      (void)n;
      (void)x_scaling;
    ...
    *
    *  @param n      (in) the number of variables \f$x\f$ in the problem; it will have the same
    * value that was specified in TNLP::get_nlp_info
    ...
    *  @param init_z (in) if true, this method must provide an initial value for the bound
    * multipliers \f$z^L\f$ and \f$z^U\f$
    ...
    *  @param m      (in) the number of constraints \f$g(x)\f$ in the problem; it will have the same
    * value that was specified in TNLP::get_nlp_info
    *  @param init_lambda (in) if true, this method must provide an initial value for the constraint
    * multipliers \f$\lambda\f$
   ...
   virtual bool get_starting_point(Index n, bool init_x, Number* x, bool init_z, Number* z_L,
         Number* z_U, Index m, bool init_lambda, Number* lambda) = 0;
   ...
   virtual bool get_warm_start_iterate(
         IteratesVector& warm_start_iterate /**< storage for warm start iterate in the form %Ipopt
   ...
   /** Method to retrieve the non-linear terms in the Jacobian
    *
    *  This information is need for CONOPT to be able to use second-order information. The
    * non-linear terms in the jacobian will inform CONOPT what to expect for the structure of the
    * Hessian of the Lagrangian. The number of non-linear terms returned from this method is at most
    * the number of elements in the Jacobian returned by `get_nlp_info`.
    *
    *  @param n     (in) the number of variables \f$x\f$ in the problem; it will have the same value
    * that was specified in TNLP::get_nlp_info
    *  @param m     (in) the number of constraints \f$g(x)\f$ in the problem; it will have the same
    * value that was specified in TNLP::get_nlp_info
    *  @param n_nl_terms (out) the number of non-linear terms in the jacobian; this is at most the
    * number of elements in the Jacobian
    *  @param iRow  (out) array of length nele_jac to store the row indices of the non-linear
    * entries in the Jacobian of the constraints
    *  @param jCol  (out) array of length nele_jac to store the column indices of the non-linear
    * entries in the Jacobian of the constraints
    *
    *  @return true if the non-linear terms are stored successfully, false otherwise. In the case of
    * false, the second order information will not be used by CONOPT.
    */
   virtual bool get_nonlinear_terms(Index n, Index m, Index& n_nl_terms, Index* iRow, Index* jCol) {
      return false;
   }
    ...
    *  @param n     (in) the number of variables \f$x\f$ in the problem; it will have the same value
    * that was specified in TNLP::get_nlp_info
    *  @param x     (in) the values for the primal variables \f$x\f$ at which the objective function
    * \f$f(x)\f$ is to be evaluated
    *  @param new_x (in) false if any evaluation method (`eval_*`) was previously called with the
    * same values in x, true otherwise. This can be helpful when users have efficient
    * implementations that calculate multiple outputs at once. %Ipopt internally caches results from
    * the TNLP and generally, this flag can be ignored.
    ...
    * f(x)\f$ is to be evaluated
    *  @param new_x (in) false if any evaluation method (`eval_*`) was previously called with the
    * same values in x, true otherwise; see also TNLP::eval_f
    *  @param grad_f (out) array to store values of the gradient of the objective function \f$\nabla
    * f(x)\f$. The gradient array is in the same order as the \f$x\f$ variables (i.e., the gradient
    * of the objective with respect to `x[2]` should be put in `grad_f[2]`).
    *
    *  @return true if success, false otherwise.
    */
    ...
    *  @param n     (in) the number of variables \f$x\f$ in the problem; it will have the same value
    * that was specified in TNLP::get_nlp_info
    *  @param x     (in) the values for the primal variables \f$x\f$ at which the constraint
    * functions \f$g(x)\f$ are to be evaluated
    *  @param new_x (in) false if any evaluation method (`eval_*`) was previously called with the
    * same values in x, true otherwise; see also TNLP::eval_f
    *  @param m     (in) the number of constraints \f$g(x)\f$ in the problem; it will have the same
    * value that was specified in TNLP::get_nlp_info
    *  @param g     (out) array to store constraint function values \f$g(x)\f$, do not add or
    * subtract the bound values \f$g^L\f$ or \f$g^U\f$.
    *
    *  @return true if success, false otherwise.
    */
   virtual bool eval_g(Index n, const Number* x, bool new_x, Index m, Number* g) = 0;

   /** Method to request either the sparsity structure or the values of the Jacobian of the
    * constraints.
    *
    ...
    *  @param n     (in) the number of variables \f$x\f$ in the problem; it will have the same value
    * that was specified in TNLP::get_nlp_info
    *  @param x     (in) first call: NULL; later calls: the values for the primal variables \f$x\f$
    * at which the constraint Jacobian \f$\nabla g(x)^T\f$ is to be evaluated
    *  @param new_x (in) false if any evaluation method (`eval_*`) was previously called with the
    * same values in x, true otherwise; see also TNLP::eval_f
    *  @param m     (in) the number of constraints \f$g(x)\f$ in the problem; it will have the same
    * value that was specified in TNLP::get_nlp_info
    *  @param nele_jac (in) the number of nonzero elements in the Jacobian; it will have the same
    * value that was specified in TNLP::get_nlp_info
    *  @param iRow  (out) first call: array of length nele_jac to store the row indices of entries
    * in the Jacobian of the constraints; later calls: NULL
    *  @param jCol  (out) first call: array of length nele_jac to store the column indices of
    * entries in the Jacobian of the constraints; later calls: NULL
    *  @param values (out) first call: NULL; later calls: array of length nele_jac to store the
    * values of the entries in the Jacobian of the constraints
    *
    *  @return true if success, false otherwise.
    *
   ...
   /** Method to request either the sparsity structure or the values of the Hessian of the
    * Lagrangian.
    *
    ...
    *  @param m     (in) the number of constraints \f$g(x)\f$ in the problem; it will have the same
    * value that was specified in TNLP::get_nlp_info
    *  @param lambda (in) the values for the constraint multipliers \f$\lambda\f$ at which the
    * Hessian is to be evaluated
    *  @param new_lambda (in) false if any evaluation method was previously called with the same
    * values in lambda, true otherwise
    *  @param nele_hess (in) the number of nonzero elements in the Hessian; it will have the same
    * value that was specified in TNLP::get_nlp_info
    *  @param iRow  (out) first call: array of length nele_hess to store the row indices of entries
    * in the Hessian; later calls: NULL
    *  @param jCol  (out) first call: array of length nele_hess to store the column indices of
    * entries in the Hessian; later calls: NULL
    *  @param values (out) first call: NULL; later calls: array of length nele_hess to store the
    * values of the entries in the Hessian
    *
    *  @return true if success, false otherwise.
    *
    * @note The arrays iRow and jCol only need to be filled once.
    ...
    * At this call, the arguments `iRow` and `jCol` will be NULL.
    *
    * @attention As this matrix is symmetric, %Ipopt expects that only the lower diagonal entries
    * are specified.
    *
   ...
   /** Return the number of variables that appear nonlinearly in the objective function or in at
    * least one constraint function
    *
   ...
   /** This method is called when the algorithm has finished (successfully or not) so the TNLP can
    * digest the outcome, e.g., store/write the solution, if any.
    *
    ...
    *   - STOP_AT_ACCEPTABLE_POINT: Algorithm stopped at a point that was converged, not to
    * "desired" tolerances, but to "acceptable" tolerances (see the acceptable-... options).
    *   - LOCAL_INFEASIBILITY: Algorithm converged to a point of local infeasibility. Problem may be
    * infeasible.
    *   - USER_REQUESTED_STOP: The user call-back function TNLP::intermediate_callback returned
    * false, i.e., the user code requested a premature termination of the optimization.
    ...
    *   - ERROR_IN_STEP_COMPUTATION: An unrecoverable error occurred while %Ipopt tried to compute
    * the search direction.
    *   - INVALID_NUMBER_DETECTED: Algorithm received an invalid number (such as NaN or Inf) from
    * the NLP; see also option check_derivatives_for_nan_inf).
   ...
   virtual void finalize_solution(SolverReturn status, Index n, const Number* x, const Number* z_L,
         ...
         const IpoptData* ip_data, IpoptCalculatedQuantities* ip_cq) = 0;
    ...
    * The method expects the number of variables (dimension of x), number of constraints (dimension
    * of g(x)), and allocated arrays of appropriate lengths as input.
    *
    * The method translates the x(), c(), d(), y_c(), y_d(), z_L(), and z_U() vectors from
    * IpoptData::curr() of the internal %NLP representation into the form used by the TNLP. For the
    * correspondence between scaled and unscaled solutions, see the detailed description of
    * OrigIpoptNLP. If %Ipopt is in restoration mode, it maps the current iterate of restoration
    * %NLP (see RestoIpoptNLP) back to the original TNLP.
    *
    * If there are fixed variables and fixed_variable_treatment=make_parameter, then requesting z_L
    * and z_U can trigger a reevaluation of the Gradient of the objective function and the Jacobian
    ...
    * @param n       (in)  the number of variables \f$x\f$ in the problem; can be arbitrary if
    * skipping x, z_L, and z_U
    * @param x       (out) buffer to store value of primal variables \f$x\f$, must have length at
    * least n; pass NULL to skip retrieving x
    * @param z_L     (out) buffer to store the lower bound multipliers \f$z_L\f$, must have length
    * at least n; pass NULL to skip retrieving z_L and z_U
    * @param z_U     (out) buffer to store the upper bound multipliers \f$z_U\f$, must have length
    * at least n; pass NULL to skip retrieving z_U and z_U
    * @param m       (in)  the number of constraints \f$g(x)\f$; can be arbitrary if skipping g and
    * lambda
    * @param g       (out) buffer to store the constraint values \f$g(x)\f$, must have length at
    * least m; pass NULL to skip retrieving g
    * @param lambda  (out) buffer to store the constraint multipliers \f$\lambda\f$, must have
    * length at least m; pass NULL to skip retrieving lambda
    *
    * @return Whether Ipopt has successfully filled the given arrays
    * @since 3.14.0
    */
   bool get_curr_iterate(const IpoptData* ip_data, IpoptCalculatedQuantities* ip_cq, bool scaled,
         Index n, Number* x, Number* z_L, Number* z_U, Index m, Number* g, Number* lambda) const {
      return false;
   }
    ...
    * of g(x)), and allocated arrays of appropriate lengths as input.
    *
    * The method makes the vectors behind (unscaled_)curr_orig_bounds_violation(),
    * (unscaled_)curr_nlp_constraint_violation(), (unscaled_)curr_dual_infeasibility(),
    * (unscaled_)curr_complementarity() from IpoptCalculatedQuantities of the internal %NLP
    * representation available into the form used by the TNLP. If %Ipopt is in restoration mode, it
    * maps the current iterate of restoration %NLP (see RestoIpoptNLP) back to the original TNLP.
    *
    ...
    *   x_L_violation and x_U_violation report the violation of a solution w.r.t. the original
    * unrelaxed bounds. However, compl_x_L and compl_x_U use the relaxed variable bounds to
    ...
    * @param n          (in)  the number of variables \f$x\f$ in the problem; can be arbitrary if
    * skipping compl_x_L, compl_x_U, and grad_lag_x
    * @param x_L_violation (out) buffer to store violation of original lower bounds on variables
    * (max(orig_x_L-x,0)), must have length at least n; pass NULL to skip retrieving orig_x_L
    * @param x_U_violation (out) buffer to store violation of original upper bounds on variables
    * (max(x-orig_x_U,0)), must have length at least n; pass NULL to skip retrieving orig_x_U
    * @param compl_x_L  (out) buffer to store violation of complementarity for lower bounds on
    * variables (\f$(x-x_L)z_L\f$), must have length at least n; pass NULL to skip retrieving
    * compl_x_L
    * @param compl_x_U  (out) buffer to store violation of complementarity for upper bounds on
    * variables (\f$(x_U-x)z_U\f$), must have length at least n; pass NULL to skip retrieving
    * compl_x_U
    * @param grad_lag_x (out) buffer to store gradient of Lagrangian w.r.t. variables \f$x\f$, must
    * have length at least n; pass NULL to skip retrieving grad_lag_x
    * @param m          (in)  the number of constraints \f$g(x)\f$; can be arbitrary if skipping
    * lambda
    * @param nlp_constraint_violation (out) buffer to store violation of constraints
    * \f$max(g_l-g(x),g(x)-g_u,0)\f$, must have length at least m; pass NULL to skip retrieving
    * constraint_violation
    * @param compl_g    (out) buffer to store violation of complementarity of constraint
    * (\f$(g(x)-g_l)*\lambda^+ + (g_l-g(x))*\lambda^-\f$, where \f$\lambda^+=max(0,\lambda)\f$ and
    * \f$\lambda^-=max(0,-\lambda)\f$ (componentwise)), must have length at least m; pass NULL to
    * skip retrieving compl_g
    *
    * @return Whether Ipopt has successfully filled the given arrays
    * @since 3.14.0
    */
   bool get_curr_violations(const IpoptData* ip_data, IpoptCalculatedQuantities* ip_cq, bool scaled,
         Index n, Number* x_L_violation, Number* x_U_violation, Number* compl_x_L,
         Number* compl_x_U, Number* grad_lag_x, Index m, Number* nlp_constraint_violation,
         Number* compl_g) const {
      return false;
   }
