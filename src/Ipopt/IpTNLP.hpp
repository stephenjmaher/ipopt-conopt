...
public:
    ...
    *
    *  This information is need for CONOPT to be able to use second-order information. The non-linear terms in the
    *  jacobian will inform CONOPT what to expect for the structure of the Hessian of the Lagrangian. The number of
    *  non-linear terms returned from this method is at most the number of elements in the Jacobian returned by
    *  `get_nlp_info`.
    *
    *  @param n     (in) the number of variables \f$x\f$ in the problem; it will have the same value that was specified in TNLP::get_nlp_info
    *  @param m     (in) the number of constraints \f$g(x)\f$ in the problem; it will have the same value that was specified in TNLP::get_nlp_info
    *  @param n_nl_terms (out) the number of non-linear terms in the jacobian; this is at most the number of elements in the Jacobian
    *  @param iRow  (out) array of length nele_jac to store the row indices of the non-linear entries in the Jacobian of the constraints
    *  @param jCol  (out) array of length nele_jac to store the column indices of the non-linear entries in the Jacobian of the constraints
    *
    *  @return true if the non-linear terms are stored successfully, false otherwise. In the case of false, the second
    *  order information will not be used by CONOPT.
    */
   virtual bool get_nonlinear_terms(
      Index   n,
      Index   m,
      Index&  n_nl_terms,
      Index*  iRow,
