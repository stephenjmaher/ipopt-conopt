...
class IPOPTLIB_EXPORT IpoptAdditionalData : public ReferencedObject {
 public:
   ...
   virtual bool Initialize(
         const Journalist& jnlst, const OptionsList& options, const std::string& prefix) = 0;
...
class IPOPTLIB_EXPORT IpoptData : public ReferencedObject {
 public:
   ...
   ///@{
   /** Constructor */
   IpoptData(SmartPtr<IpoptAdditionalData> add_data = NULL) : add_data_(add_data) {
      ...
      info_regu_x_ = 0.0;
      info_alpha_primal_ = 0.0;
      info_alpha_primal_char_ = ' ';
      info_alpha_dual_ = 0.0;
      info_ls_count_ = 0;
      info_skip_output_ = false;
      info_last_output_ = -1.0;
      info_iters_since_header_ = 1000;  // Need to be larger than 10
      have_deltas_ = false;
   ...
   bool InitializeDataStructures(IpoptNLP& ip_nlp, bool want_x, bool want_y_c, bool want_y_d,
      ...
      have_prototypes_ = true;  // Mark as available for compatibility
      have_deltas_ = false;
      have_affine_deltas_ = false;

      // Initialize additional data if present
      bool retval = true;
   ...
   bool Initialize(const Journalist& jnlst, const OptionsList& options, const std::string& prefix) {
      // Get tolerance from options
      options.GetNumericValue("tol", tol_, prefix);

      // Reset iteration and algorithm state
      iter_count_ = 0;
      curr_mu_ = -1.0;
      mu_initialized_ = false;
      curr_tau_ = -1.0;
   ...
   SmartPtr<const IteratesVector> trial() const {
      DBG_ASSERT(IsNull(trial_) ||
            (trial_->GetTag() == debug_trial_tag_ && trial_->GetTagSum() == debug_trial_tag_sum_));
   ...
   void SetTrialPrimalVariablesFromStep(
      ...
      // 1. Create or get trial_ IteratesVector
      // 2. Create new x = curr_->x() + alpha * delta_x
   ...
   void SetTrialEqMultipliersFromStep(
      ...
      // This is a stub for compatibility with Ipopt interface
      // In a full Ipopt implementation, this would:
      // 1. Get trial_ IteratesVector (must exist from SetTrialPrimalVariablesFromStep)
      // 2. Create new y_c = curr_->y_c() + alpha * delta_y_c
    ...
    *  alpha.
    */
   void SetTrialBoundMultipliersFromStep(Number alpha, const Vector& delta_z_L,
      ...
      // In a full Ipopt implementation, this would:
      // 1. Get trial_ IteratesVector (must exist from SetTrialPrimalVariablesFromStep)
      // 2. Create new z_L = curr_->z_L() + alpha * delta_z_L
      // 3. Create new z_U = curr_->z_U() + alpha * delta_z_U
      // 4. Create new v_L = curr_->v_L() + alpha * delta_v_L
   ...
   SmartPtr<const IteratesVector> delta() const {
      DBG_ASSERT(IsNull(delta_) ||
            (delta_->GetTag() == debug_delta_tag_ && delta_->GetTagSum() == debug_delta_tag_sum_));
   ...
   void set_delta(SmartPtr<IteratesVector>& delta) {
      ...
      // Set delta_ to NULL since CONOPT doesn't use this
      delta_ = NULL;
   ...
   SmartPtr<const IteratesVector> delta_aff() const {
      DBG_ASSERT(IsNull(delta_aff_) ||
            (delta_aff_->GetTag() == debug_delta_aff_tag_ &&
                  delta_aff_->GetTagSum() == debug_delta_aff_tag_sum_));
   ...
   inline void set_delta_aff(SmartPtr<IteratesVector>& delta_aff) {
      ...
      // Set delta_aff_ to NULL since CONOPT doesn't use this
      delta_aff_ = NULL;
   ...
   inline void CopyTrialToCurrent() {
      curr_ = trial_;
#if IPOPT_CHECKLEVEL > 0
   ...
   void AcceptTrialPoint() {
      ...
      // Call additional data AcceptTrialPoint if present
      if (IsValid(add_data_)) {
   ...
   static void RegisterOptions(const SmartPtr<RegisteredOptions>& roptions) {
      // Register options for IpoptData
      roptions->SetRegisteringCategory("Termination");
      roptions->AddLowerBoundedNumberOption("tol", "Desired convergence tolerance (relative).", 0.,
            true, 1e-8,
            "Determines the convergence tolerance for the algorithm. "
            "The algorithm terminates successfully, if the (scaled) NLP error becomes smaller than "
            "this value, and "
            "if the (absolute) criteria according to \"dual_inf_tol\", \"constr_viol_tol\", and "
            "\"compl_inf_tol\" are met. "
            "This is epsilon_tol in Eqn. (6) in implementation paper. "
            "See also \"acceptable_tol\" as a second termination criterion. "
            "Note, some other algorithmic features also use this quantity to determine thresholds "
            "etc.");
   }
