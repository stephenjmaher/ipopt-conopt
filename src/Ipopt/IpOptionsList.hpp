...
namespace Ipopt {
   ...
   public:
       ...
       */
      bool GetConoptOption(int index, std::string& name, double* rval, int* ival, int* lval, ConoptOption::Type* type) const {
         ...
         switch (opt.type) {
            case ConoptOption::INTEGER:
               if (ival) *ival = opt.value.ival;
               break;
            case ConoptOption::REAL:
               if (rval) *rval = opt.value.rval;
               break;
            case ConoptOption::LOGICAL:
               if (lval) *lval = opt.value.lval;
               break;
         }
         return true;
      }
      ...
      bool ApplyToConopt(coiHandle_t cntvect, Journalist* jnlst) {
         ...
         for (const auto& pair : string_options_) {
            ...
            if (name == "derivative_test") {
               int conopt_val = 0;
               if (value == "first-order" || value == "first-point") conopt_val = -1;
               if (value == "only-some-derivatives") conopt_val = 1; /* Or some other value */
